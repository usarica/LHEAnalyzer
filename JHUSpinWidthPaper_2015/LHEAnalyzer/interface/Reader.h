...
public:
  ...
  template<typename returnType> void addFunction(string branchname, returnType(*evalVar)(const Event*, string&)){
    int varposition=-1;
    ...
    if (varposition==-1 || varbranchtype==BaseTree::nBranchTypes){
      ...
      if (varbranchtype==BaseTree::bInt) { pair<string, Int_t(*)(const Event*, string&)> varPair(branchname, evalVar); intFunctions.push_back(varPair); }
      else if (varbranchtype==BaseTree::bFloat) { pair<string, Float_t(*)(const Event*, string&)> varPair(branchname, evalVar); floatFunctions.push_back(varPair); }
      else if (varbranchtype==BaseTree::bVectorInt) { pair<string, vectorInt(*)(const Event*, string&)> varPair(branchname, evalVar); vectorIntFunctions.push_back(varPair); }
      else if (varbranchtype==BaseTree::bVectorDouble) { pair<string, vectorDouble(*)(const Event*, string&)> varPair(branchname, evalVar); vectorDoubleFunctions.push_back(varPair); }
    }
  }
...
protected:
  void configure();
  void finalizeRun();
  void readEvent(Event& outEvent, bool isGen);
  ...
  vector < pair<string, vectorInt(*)(const Event*, string&)> > vectorIntFunctions;
  vector < pair<string, vectorDouble(*)(const Event*, string&)> > vectorDoubleFunctions;
