...
void regularizeTxySyst(TH1F* txy_syst[3]);
...
float get_TxySyst(float val, float err, int kSystVar=0){
...
float compute_TxyAlternatives(float Txy, float Txy_true, float systVar){
  return (Txy-Txy_true)*systVar+Txy_true;
}
...
void makeCombineSignalTemplateswithTrees_Signal_single(int folder, int erg_tev, int iProd, int scheme){
  ...
  TString OUTPUT_NAME = "_templates_Sig_";
  ...
  float delTxy=-99;
  float Dxy=-99, delDxy=-99;
  float Txy_true=-99, Dxy_true=-99;
  ...
  TString INPUT_SYST_NAME = "_templateSystematics_Sig_";
  ...
  for (int genctau = 0; genctau <= nctaus; genctau++){
    ...
    if (genctau>0){
      ...
      //      cout << "Extracted Txy syst histogram " << Form("fit_CTau%.0f__KD", target_ctau) << endl;
      ...
      //      cout << "Extracted Txy syst histogram " << Form("H_2D_CTau%.0f_px", target_ctau) << endl;
      for (int st=0; st<3; st++){
        if (txy_syst[st]==0) cout << "txy_syst[" << st << "] empty" << endl;
        else cout << "Txy syst " << st << " norm: " << txy_syst[st]->Integral() << endl;
      }
    }
    regularizeTxySyst(txy_syst);
...
//    mytree->Branch("KD_dn", &KD_dn);
    ...
    for (int f = 0; f < kNumSamples; f++){
      ...
      for (int ev = 0; ev < tree[f]->GetEntries(); ev++){
        ...
        //        float TxySystwgt_up = get_TxySyst(smearingVal, smearingErr, 1);
        //        float TxySystwgt_dn = get_TxySyst(smearingVal, smearingErr, -1);
        //        float TxySystwgt_nominal = get_TxySyst(smearingVal, smearingErr, 0);

        //        KD = compute_TxyAlternatives(Txy, Txy_true, TxySystwgt_nominal);
        KD = Txy;
        KD = tanh(KD / KD_scale); // Scaled to due to tanh function
        if (KD >= 1.) KD = 1. - 1.0e-4;
        if (KD <= -1.) KD = -1. + 1.0e-4;
        /*
        KD_up = compute_TxyAlternatives(Txy, Txy_true, TxySystwgt_up);
        KD_up = tanh(KD_up / KD_scale); // Scaled to due to tanh function
        if (KD_up >= 1.) KD_up = 1. - 1.0e-10;
        if (KD_up <= -1.) KD_up = -1. + 1.0e-10;

        KD_dn = compute_TxyAlternatives(Txy, Txy_true, TxySystwgt_dn);
        ...
        */
        for (int ss = 0; ss < nSyst; ss++){
          ...
          double TxySystWgt=1;
          int txy_syst_bin = txy_syst[0]->GetXaxis()->FindBin(Txy);
          if (txy_syst_bin > txy_syst[0]->GetNbinsX()) txy_syst_bin = txy_syst[0]->GetNbinsX();
          if (txy_syst_bin < 1) txy_syst_bin = 1;
          if (ss == (kSystNominal + iSystTxy)) TxySystWgt = txy_syst[1]->GetBinContent(txy_syst_bin);
          else if (ss == (kSystNominal - iSystTxy)) TxySystWgt = txy_syst[2]->GetBinContent(txy_syst_bin);
          if (TxySystWgt>1.6) TxySystWgt=1.6;
          if (TxySystWgt<0.6) TxySystWgt=0.6;
          if (TxySystWgt!=TxySystWgt) TxySystWgt = 1;
          templateWeight[ss] *= TxySystWgt;
          //          if ((ss == (kSystNominal + iSystTxy) || ss == (kSystNominal - iSystTxy)) && Txy>1000 && target_ctau==600.) cout << Txy << '\t' << TxySystWgt << endl;

          //          if (ss == (kSystNominal + iSystTxy)) hfill[ss]->Fill(KD_up, smd[ss], templateWeight[ss]);
          //          else if (ss == (kSystNominal - iSystTxy)) hfill[ss]->Fill(KD_dn, smd[ss], templateWeight[ss]);
    ...
    for (int ss = 0; ss < nSyst; ss++){
      ...
      */
      ...
      hfill_projX->SetNameTitle(Form("H_1DTxy_CTau%.0f%s", target_ctau, cSyst[ss]), Form("%s Projection on tanh(T_{xy} / %.0f #mum)", hfill[ss]->GetTitle(), KD_scale));
    ...
    TH1F* hTxySystRatio[2];
    hTxySystRatio[0] = (TH1F*)hprojX_array[(kSystNominal + iSystTxy)]->Clone(Form("%s_Ratio", hprojX_array[(kSystNominal + iSystTxy)]->GetName()));
    hTxySystRatio[1] = (TH1F*)hprojX_array[(kSystNominal - iSystTxy)]->Clone(Form("%s_Ratio", hprojX_array[(kSystNominal - iSystTxy)]->GetName()));
    for (int rr=0; rr<2; rr++){
      hTxySystRatio[rr]->Divide(hprojX_array[kSystNominal]);
      foutput->WriteTObject(hTxySystRatio[rr]);
      delete hTxySystRatio[rr];
    }
    for (int ss = 0; ss < nSyst; ss++){
...
TH2F** makeCombineSignalTemplateMCSyst_Signal_single(int folder, int erg_tev, int iProd, int scheme){
  ...
  for (int genctau = 0; genctau <= nctaus; genctau++){
    ...
    hfill->SetTitle("");
    hfill->GetXaxis()->SetTitle("T_{xy} (#mum)");
...
void makeCombineSignalTemplateSyst_Signal_single(int folder, int erg_tev, int iProd, int scheme){
  ...
  char TREE_NAME[]="SelectedTree";
  TString INPUT_NAME = "HZZ4lTree_powheg15jhuGenV3-CTau";
  TString OUTPUT_NAME = "_templateSystematics_Sig_";
  if (iProd>0) OUTPUT_NAME.Append(Form("%s_", strHiggsProduction[iProd-1].Data()));
  OUTPUT_NAME = OUTPUT_NAME + cutNames[scheme] + ".root";
  OUTPUT_NAME.Prepend(user_folder[folder]);
  TString comstring;
  ...
  for (int genctau = 0; genctau <= 2*nctaus; genctau++){
    ...
    for (int ss = 0; ss < nSyst; ss++){
      hfill[ss] = new TH2F(Form("H_2D_CTau%.0f%s", target_ctau, cSyst[ss]), "", nbinsx, xlow, xhigh, nbinsy, ylow, yhigh);
      hfill[ss]->SetTitle(Form("%.0f #mum %s at %i TeV", target_ctau, user_folder[folder], erg_tev));
      hfill[ss]->GetXaxis()->SetTitle("T_{xy} #mum");
      hfill[ss]->GetYaxis()->SetTitle("#it{D}_{bkg}");
    ...
    for (int ev = 0; ev < tree->GetEntries(); ev++){
      ...
      for (int ss = 0; ss < nSyst; ss++){
        float KD_fill;
        if (ss == (kSystNominal + iSystTxy)) KD_fill=KD_up;
        else if (ss == (kSystNominal - iSystTxy)) KD_fill=KD_dn;
        else KD_fill=KD;
    ...
    if ((genctau % 2)==0){
      ...
      hProj_container[kSystNominal + iSystTxy]->Scale(luminosity[EnergyIndex]);
      hProj_container[kSystNominal - iSystTxy]->Scale(luminosity[EnergyIndex]);
      ...
      if (genctau>0){
        RooRealVar* frac1 = new RooRealVar(Form("frac1_CTau%.0f", target_ctau), "", 0.5, 0, 1);
      ...
      if (hFit!=0){
        ...
        hFit_up = (TH1F*)hProj_container[kSystNominal + iSystTxy]->Clone(Form("%s_up", hFit->GetName()));
        hFit_up->Scale(n1val);
        hFit_up->Add(hProj_grandcontainer[genctau_2][kSystNominal + iSystTxy], n2val);
        hFit_up->Add(hProj_grandcontainer[0][kSystNominal + iSystTxy], 1.-n2val-n1val);

        hFit_dn = (TH1F*)hProj_container[kSystNominal - iSystTxy]->Clone(Form("%s_dn", hFit->GetName()));
        hFit_dn->Scale(n1val);
        hFit_dn->Add(hProj_grandcontainer[genctau_2][kSystNominal - iSystTxy], n2val);
        hFit_dn->Add(hProj_grandcontainer[0][kSystNominal - iSystTxy], 1.-n2val-n1val);
      ...
      double maxplot = max(hMC_proj->GetMaximum(), hProj_container[kSystNominal - iSystTxy]->GetMaximum());
      ...
      hProj_container[kSystNominal - iSystTxy]->SetLineColor(kRed);
      hProj_container[kSystNominal - iSystTxy]->SetLineWidth(2);
      hProj_container[kSystNominal - iSystTxy]->SetLineStyle(7);
      hProj_container[kSystNominal + iSystTxy]->SetLineColor(kGreen+2);
      hProj_container[kSystNominal + iSystTxy]->SetLineWidth(2);
      hProj_container[kSystNominal + iSystTxy]->SetLineStyle(7);
      ...
      if (hFit!=0){
        hFit_up->SetLineColor(hProj_container[kSystNominal + iSystTxy]->GetLineColor());
        hFit_up->SetLineWidth(2);
        hFit_up->SetLineStyle(7);

        hFit_dn->SetLineColor(hProj_container[kSystNominal - iSystTxy]->GetLineColor());
      ...
      TString canvasname = "cCompareLifetimeTxySyst_";
      ...
      if (genctau!=0) hMC_proj->GetXaxis()->SetRangeUser(-800, 1600);
      else hMC_proj->GetXaxis()->SetRangeUser(-1400, 1400);
      hMC_proj->Draw("hist");
      if (genctau==0){
        ll->AddEntry(hProj_container[kSystNominal + iSystTxy], "Resolution up", "l");
        ll->AddEntry(hProj_container[kSystNominal - iSystTxy], "Resolution down", "l");
        hProj_container[kSystNominal + iSystTxy]->Draw("histsame");
        hProj_container[kSystNominal - iSystTxy]->Draw("histsame");
      }
      else if (hFit!=0){
        ll->AddEntry(hProj_container[kSystNominal], "Convoluted c#tau=0 #mum (nominal)", "l");
        ll->AddEntry(hFit, "3-convolution fit (nominal)", "l");
        ll->AddEntry(hFit_up, "3-convolution (res. up)", "l");
        ll->AddEntry(hFit_dn, "3-convolution (res. down)", "l");
...
void makeCombineSignalTemplateswithTrees_Bkg_single(int folder, int erg_tev, int Systematics, int scheme){
  /*	if (Systematics !=0){
  cout << "Txy up and down are not yet supported" << endl;
  return;
  }
  */
  char TREE_NAME[]="SelectedTree";
  TString OUTPUT_NAME = "_templates_background_" + cutNames[scheme] + "_";
  OUTPUT_NAME.Prepend(user_folder[folder]);
  TString comstring;
  comstring.Form("%iTeV", erg_tev);
  TString erg_dir;
  ...
  for (int tt = 0; tt < ntrees; tt++){
    ...
    for (int ev = 0; ev < tc[tt]->GetEntries(); ev++){
      ...
      float TxySystwgt = 1;
      if (tt!=2) TxySystwgt = get_TxySyst(smearingVal, smearingErr, Systematics);
      KD = compute_TxyAlternatives(Txy, Txy_true, TxySystwgt);
      KD = tanh(KD / KD_scale); // Scaled to due to tanh function
      if (KD >= 1.) {
        KD = 1. - 1e-4;
        counter_edges++;
      ...
      //      if (tt==2) cout << ZZMass << '\t' << Txy << '\t' << MC_weight_noxsec << '\t' << CRflag << endl;
    ...
    for (int ev = 0; ev < tc[tt]->GetEntries(); ev++){
      ...
      KD = tanh(KD / KD_scale); // Scaled to due to tanh function
      if (KD >= 1.) {
        KD = 1. - (binwidthx*smeared_edges/counter_edges*0.1);
        smeared_edges++;
      }
      if (KD <= -1.) {
        KD = -1. + (binwidthx*smeared_edges/counter_edges*0.1);
