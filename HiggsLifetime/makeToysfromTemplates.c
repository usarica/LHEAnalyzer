...
TString templateDir = "/scratch0/hep/usarical/HiggsLifetime/Analysis/CMSSW_6_1_1/src/HiggsAnalysis/HZZ4l_Combination/CreateDatacards/templates2D/HCTau_NewSIP/";
...
void makeCombineAsimovTrees_NoSIP(int isBkg = 0, int selection=4){
  for (int CoM=7; CoM<9; ++CoM){
    for (int channel=0; channel<3; channel++){
      ...
      else makeCombineSignalAsimovTrees_Bkg_single(channel, CoM, selection);
    }
  }
  for (int iProd=0; iProd<nHiggsProd; iProd++){
    for (int BSMSample=0; BSMSample<=kfLambda1_05; BSMSample++){
...
void makeCombineSignalAsimovTrees_Signal_single(int folder, int erg_tev, int iProd, int BSMSample, int BSMNormScheme, int scheme){
  ...
  TString OUTPUT_NAME = "AsimovTree_Sig_";
...
void makeCombineSignalAsimovTrees_Bkg_single(int folder, int erg_tev, int scheme){
  ...
  char TREE_NAME[]="SelectedTree";
  TString OUTPUT_NAME = "AsimovTree_background_";
  OUTPUT_NAME.Append(user_folder[folder]);
  OUTPUT_NAME.Append("_");
  OUTPUT_NAME.Append(comstring);
...
void assembleCombineEmbeddedToys(double target_ctau, int BSMSample, int iProd, double BSMNormScheme, int scheme){
  ...
  TString OUTPUT_NAME = "AsimovToys_Hypo";
  ...
  for (int erg_tev=7; erg_tev<=8; erg_tev++){
    ...
    for (int folder=0; folder<3; folder++){
      ...
      TString INPUT_NAME = "AsimovTree_background_";
  ...
  for (int kk=0; kk<N_KDs+N_Kinematics; kk++){
    ...
    RooPlot* valPlot = new RooPlot(cValPlot, "", *rCMS_zz4l_VarContainer[kk], rCMS_zz4l_VarContainer[kk]->getMin(), rCMS_zz4l_VarContainer[kk]->getMax(), 80);
...
void compareEmbeddedAsimov_SignalTemplates(double target_ctau, int iProd, int scheme){
  ...
  TString xTitle = "tanh(T_{xy} / 800 #mum)";

  TFile* finput = 0;

  TString OUTPUT_NAME;
  if (!isBkg){
    OUTPUT_NAME = "AsimovToys_SignalComparison";
    OUTPUT_NAME.Append("_");
    OUTPUT_NAME.Append(Form("CTau%.0f", target_ctau));
    if (iProd>0) OUTPUT_NAME.Append(Form("_%s", strHiggsProduction[iProd-1].Data()));
    ...
    OUTPUT_NAME = "AsimovToys_BackgroundComparison";
  ...
  for (int erg_tev=7; erg_tev<=8; erg_tev++){
    ...
    for (int folder=0; folder<3; folder++){
      ...
      foutput->cd();
      TH1F* htemplate = (TH1F*)htemp->Clone(Form("%s_clone", htemp->GetName()));
      delete htemp;
      htemp = (TH1F*)finput->Get("T_2D_TxyNominal_ttH");
      TH1F* htemplate_tth = (TH1F*)htemp->Clone(Form("%s_clone", htemp->GetName()));
      delete htemp;
      finput->Close();
      ...
      for (int BSMSample=0; BSMSample<kNumGGHVVBSM; BSMSample++){
        ...
        if (!isBkg){
          ...
          INPUT_NAME = "AsimovTree_background_";
          INPUT_NAME.Append(user_folder[folder]);
          INPUT_NAME.Append("_");
          INPUT_NAME.Append(comstring);
          INPUT_NAME =  INPUT_NAME + "_" + cutNames[scheme] + ".root";
        ...
        hAsimov[BSMSample] = (TH1F*)htemplate->Clone(Form("%s_Asimov_BSM%i", htemplate->GetName(), BSMSample));
        ...
        hAsimov_fill[BSMSample] = (TH1F*)hAsimov[BSMSample]->Clone(Form("%s_Fill", hAsimov[BSMSample]->GetName(), BSMSample));
      ...
      if (iProd>0 && !isBkg && target_ctau==0) max_plot = max(max_plot, htemplate_tth->GetMaximum());
      htemplate->GetYaxis()->SetRangeUser(0, max_plot*1.3);
      if (target_ctau==0 && !isBkg && iProd>0) htemplate->GetYaxis()->SetRangeUser(1e-5, 1.);
      else if (isBkg || target_ctau==0 && !isBkg && iProd==0 && folder!=1) htemplate->GetYaxis()->SetRangeUser(1e-4, 1.);
      else if (target_ctau==0 && !isBkg && iProd==0 && folder==1) htemplate->GetYaxis()->SetRangeUser(1e-3, 1.);
      ...
      foutput->cd();
      TString canvasname_2D = "cCompare_EmbeddedAsimov_Templates_";
      ...
      htemplate->Draw("hist");
      /*      if (iProd>0 && iProd<4 && !isBkg && target_ctau==0){
      ...
      */      for (int BSMSample=0; BSMSample<kNumGGHVVBSM; BSMSample++){
        if (hAsimov[BSMSample]!=0){
          ...
          embeddedLabel.Prepend("Injected ");
      ...
      else if (folder==2) folder_id = "2e+2#mu";
