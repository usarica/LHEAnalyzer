...
void computeOSWeights_CR(int erg_tev=8, int iCR=0, double m4l_low = 100, double m4l_high = 3000);

void combineOSWeights(int erg_tev=8, double m4l_low=100, double m4l_high=3000);
...
double applyOSSFweight(
  ...
  if (LepcombRelIsoPF>=0.4 || !LepisID){
    if (abs(LepID)==13 && LepEta<1.2) histoindex=0;
    if (abs(LepID)==13 && LepEta>=1.2) histoindex=1;
    if (abs(LepID)==11 && LepEta<1.45) histoindex=2;
    if (abs(LepID)==11 && LepEta>=1.45) histoindex=3;
  }
  if (histoindex>=0){
    OSSFweight = hOS[histoindex]->GetBinContent(hOS[histoindex]->GetXaxis()->FindBin(LepPt));
    OSweighterror = hOS[histoindex]->GetBinError(hOS[histoindex]->GetXaxis()->FindBin(LepPt));
  }
...
void computeOSWeights(int erg_tev=8, double m4l_low = 100, double m4l_high = 3000){
  for (int iCR=0; iCR<=2; iCR++) computeOSWeights_CR(erg_tev, iCR, m4l_low, m4l_high);
  combineOSWeights(erg_tev, m4l_low, m4l_high);
}
